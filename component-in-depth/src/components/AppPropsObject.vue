...
const fruits = reactive([
  ...
  { id: 'f2', name: '파인애플', checked: false, price: 2000, origin: '필리핀' },
  { id: 'f3', name: '포도', checked: false, price: 1500, origin: '이탈리아' },
  { id: 'f4', name: '딸기', checked: true, price: 2500, origin: '미국' },
  { id: 'f5', name: '아보카도', checked: false, price: 3000, origin: '멕시코' },
  { id: 'f6', name: '메론', checked: false, price: 4000, origin: '터키' },
]);

function toggleFruit(id) {
  const fruit = fruits.find((f) => f.id === id);
  if (fruit) fruit.checked = !fruit.checked;
}

function showSelectedFruit(name) {
  alert(`선택한 과일: ${name}`);
}

const selectedCount = computed(() => fruits.filter((f) => f.checked).length);
</script>
  ...
  <div>
    ...
    <div class="fruits">
      ...
      <AppPropsObjectChild
        ...
        :fruit="fruit"
        @toggle="toggleFruit"
        @select="showSelectedFruit"
      />
      <p>현재 선택된 과일: {{ selectedCount }}개</p>
